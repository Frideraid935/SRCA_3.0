...
const calificacionesController = {
    ...
    // =========================
    // Registrar calificación (sin cambios)
    // =========================
    ...
    // =========================
    // Buscar calificación por alumno + materia (sin cambios)
    // =========================
    ...
    // =========================
    // Actualizar calificación usando id de materia
    // =========================
    async actualizar(req, res) {
        try {
            const { id } = req.params;
            const { alumno_nombre, numero_de_control, materia_id, calificacion, profesor_nombre } = req.body;

            if (!alumno_nombre || !numero_de_control || !materia_id || !calificacion || !profesor_nombre) {
                return res.status(400).json({ message: 'Todos los campos son obligatorios' });
            }

            // Obtener numero_de_control del profesor
            const [profesor] = await db.query('SELECT numero_de_control FROM profesores WHERE nombre = ?', [profesor_nombre]);
            if (!profesor.length) return res.status(404).json({ message: 'Profesor no encontrado' });
            ...
            const [result] = await db.query(sql, [
                alumno_nombre,
                numero_de_control,
                materia_id,      // <-- Usando id de materia directamente
                calificacion,
                profesor[0].numero_de_control,
                id
            ]);
